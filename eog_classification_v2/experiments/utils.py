...
def evaluate(model, test_batch, test_targets, cm=False):
    correct = 0
    i = 0
    confusion_matrix = np.zeros((len(test_targets[0]), len(test_targets[0])))
...
def experiment(cfg, real_data, reference_data, zero_shot_cls=None, cm=False):
    ...
    for e in range(cfg.epochs):
        ...
        test_acc, _ = evaluate(model, test_batch, test_targets)
        ...
        if cm==True and e == cfg.epochs-1:
            print('create confusion matrix')
            _, confusion_matrix = evaluate(model, test_batch, test_targets, cm=True)
                
            return model, train_acc_list, train_loss_list, test_acc_list, confusion_matrix
